...
int exportDmaBufFd(void *devPtr, size_t size) {
    ...
    // The IPC handle on Linux with the xe/i915 driver contains a DMA-BUF fd
    // in the first sizeof(int) bytes of the opaque handle data.
    int fd = -1;
    memcpy(&fd, ipc_handle.data, sizeof(fd));
    if (fd < 0) {
        LOG(ERROR) << "DMA-BUF fd extraction failed (fd=" << fd << ")";
        return -1;
    }
    return fd;
}
